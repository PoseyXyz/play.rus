        ...
        <div className='relative mini-card'>
            <div className='absolute w-full h-full z-10 flex flex-col items-center justify-center overlay'>
                <div className='flex flex-col gap-2 text-center'>
                    <h1 className='text-xl font-sans font-bold'>{name} {id} {slug} {parseSlug()}</h1>
                    <button onClick={()=>{fetchGamesList(type, parseSlug()); navigate(`/games/${parseSlug}`)}} className='py-2.5 px-4 btn-overlay self-center rounded-lg hover:bg-white hover:text-black duration-150 delay-75 font-semibold'>View Games</button>
                </div>
